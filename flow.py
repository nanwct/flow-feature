...
# from tempfile import TemporaryFile #tempfile类型没法搞定csv，tempfile如果需要同时读写，则需以“w+b”形式打开文件，但这种情况下csv.writer写过的文件似乎无法重新读取。
...
feature_name = ['fiat_mean', 'fiat_min', 'fiat_max', 'fiat_std', 'biat_mean', 'biat_min', 'biat_max', 'biat_std',
                ...
                'fpl_s', 'bpl_s', 'dpl_s', 'fin_cnt', 'syn_cnt', 'rst_cnt', 'pst_cnt', 'ack_cnt', 'urg_cnt', 'cwe_cnt',
                'ece_cnt',
...
tempfiles = []
temp_dir = "temp"
# pcapname_complier = re.compile(".*/(?P<domain>.*)_(?P<doh_uri>.*)_(?P<broswer>.*)_(?P<time>.*)\.pcap")
...
class flowProcess(Process):
    ...
    def __init__(self, writer_index: int, read_pcap, process_name=None):
    ...
    def run(self):
        ...
        for pcap_name in self.pcaps:
            self.read_pcap(pcap_name, self.writer_index)
...
def get_flow_feature_from_pcap(pcapname, writer_index):
    try:
    ...
    for flow in flows.values():
        ...
        csv_writers[writer_index].writerow(feature)


# get a pcap feature
def get_pcap_feature_from_pcap(pcapname, writer_index):
    try:
        # It is possible that scapy can not read the pcap
        packets = rdpcap(pcapname)
    except Exception as e:
...
def get_flows_from_pcap(pcapname):
    try:
        # It is possible that scapy can not read the pcap
        packets = rdpcap(pcapname)
    except Exception as e:
